...
ssh -q $CLIENT_HOST -- curl -X POST http://$ENTRY_HOST:8080/function/VoidInit

ssh -q $CLIENT_HOST -- docker run -v /tmp:/tmp \
    askiad/boki-voidbench:sosp-ae \
    cp /voidbench-bin/create_users /tmp/create_users
...
ssh -q $CLIENT_HOST -- docker run -v /tmp:/tmp \
    askiad/boki-voidbench:sosp-ae \
    cp /voidbench-bin/benchmark /tmp/benchmark
